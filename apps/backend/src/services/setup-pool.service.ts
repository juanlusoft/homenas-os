 ...
 * The backend runs as 'homenas' (non-root) so plain writeFileSync to /etc/*
...
async function writeSnapraidConf(dataMounts: string[], parityMounts: string[]): Promise<void> {
  ...
  // /etc/snapraid.conf is root-owned (same EACCES path as /etc/fstab).
  await writeRootFile('/etc/snapraid.conf', conf)
}
...
export async function configurePool(config: PoolConfig): Promise<void> {
  ...
  if (poolType === 'snapraid') await writeSnapraidConf(dataMounts, parityMounts)
